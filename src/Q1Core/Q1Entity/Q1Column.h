...
public:
    ...
    static Q1ColumnDataType GetColumnType(const QString& sql_type)
    {
        ...
        if (type == "integer" || type == "int" || type == "int4")
            ...
            return REAL;
        else if (type == "double precision" || type == "float8" || type == "float")
            return DOUBLE_PRECISION;
        else if (type == "boolean" || type == "bool" || type == "bit")
            return BOOLEAN;
        else if (type == "character" || type.startsWith("character(") || type == "char" ||
                 type == "nchar")
            return CHAR;
        else if (type == "text")
            return TEXT;
    ...
    static bool IsIdentityDefault(const QString& default_value)
    {
        const QString normalized = default_value.trimmed();

        return normalized.compare("GENERATED ALWAYS AS IDENTITY", Qt::CaseInsensitive) == 0 ||
               normalized.compare("GENERATED BY DEFAULT AS IDENTITY", Qt::CaseInsensitive) == 0 ||
               normalized.contains("nextval(", Qt::CaseInsensitive) ||
               normalized.contains("serial", Qt::CaseInsensitive) ||
               normalized.contains("autoincrement", Qt::CaseInsensitive) ||
               normalized.contains("identity", Qt::CaseInsensitive);
    }

    static QString NormalizeDefaultValue(QString default_value)
    {
        default_value = default_value.trimmed();

        while (default_value.startsWith('(') && default_value.endsWith(')') && default_value.size() > 1)
        {
            default_value = default_value.mid(1, default_value.size() - 2).trimmed();
        }

        if (default_value.compare("null", Qt::CaseInsensitive) == 0)
            return QString();

        if (IsIdentityDefault(default_value))
            return "GENERATED ALWAYS AS IDENTITY";

        return default_value;
    }

    static bool DefaultsMatch(const QString& left, const QString& right)
    {
        return NormalizeDefaultValue(left).compare(NormalizeDefaultValue(right), Qt::CaseInsensitive) == 0;
    }
