...
namespace
{
void conciseMessageHandler(QtMsgType type,
...
public:
    ...
    {
        return {entity.Relations("countries", "cities", ONE_TO_MANY, "country_id", "id")};
    }
};
...
public:
    ...
    {
        return {entity.Relations("cities", "countries", MANY_TO_ONE, "country_id", "id")};
    }
};
...
public:
    explicit ExampleContext(Q1Connection* conn)
        : cities(conn), countries(conn)
    {
        // Configure the entities before Q1Context creates or checks tables.
...
protected:
    ...
    {
        return {cities.GetTablePtr(), countries.GetTablePtr()};
    }

    QList<Q1Relation> OnTableRelationCreating() override
    {
        return CityMap::CreateRelations(cities);
    }
};

const DbConfig dbs[] = {
    {Q1Driver::POSTGRE_SQL, "PostgreSQL", "PG", "postgres", "123", "q1orm_test", 5432},
    {Q1Driver::SQLSERVER, "SQLServer", "SQLSERVER", "sa", "123", "q1orm_test", 1433},
    {Q1Driver::MYSQL, "MySQL", "MYSQL", "root", "P@nd1378", "q1orm_test2", 3306},
...
{
    const QString prefix = QStringLiteral("Q1ORM_%1_").arg(QString::fromLatin1(db.envTag).toUpper());
    return {
        db.driver,
        env((prefix + "HOST").toLocal8Bit().constData(), env("Q1ORM_DB_HOST", "localhost")),
        env((prefix + "DB_NAME").toLocal8Bit().constData(), env("Q1ORM_TEST_DB_NAME", db.database)),
        env((prefix + "USER").toLocal8Bit().constData(), env("Q1ORM_DB_USER", db.user)),
        env((prefix + "PASSWORD").toLocal8Bit().constData(), env("Q1ORM_DB_PASSWORD", db.password)),
        envInt((prefix + "PORT").toLocal8Bit().constData(), envInt("Q1ORM_DB_PORT", db.port))
    };
}

QString color(const QString& text, const char* code)
{
    return QStringLiteral("\033[%1m%2\033[0m").arg(QString::fromLatin1(code), text);
}
...
struct Suite
{
    explicit Suite(const DbConfig& config)
        : config(config), settings(ResolveSettings(config)),
          conn(settings.driver, settings.host, settings.database, settings.user, settings.password, settings.port),
          ctx(&conn)
    {
    }
    ...
    static void showcaseValue(const QString& label, const QVariant& value)
    {
        qInfo().noquote() << QStringLiteral("  %1 : %2")
                                 .arg(label.leftJustified(24, ' '), value.toString());
    }
    ...
    bool showcase()
    {
        ...
        if (!migration.ErrorMessage().isEmpty())
            return fail("showcase migration", migration.ErrorMessage());
        qInfo().noquote() << QStringLiteral("[MIGRATION] history table ready | %1 table(s) | %2 country column(s)")
                                 .arg(tables.size())
                                 .arg(countryColumns.size());
        qInfo().noquote() << QStringLiteral("[MIGRATION] visible database(s): %1")
                                 .arg(databases.join(", "));
        qInfo().noquote() << "[MIGRATION] Context.Initialize() synchronized tables and relations";
        ...
        for (const Q1Column& column : ctx.countries.GetTableColumns())
            qInfo().noquote() << QStringLiteral("  - %1 (%2)%3")
                                     .arg(column.name,
                                          QString::number(static_cast<int>(column.type)),
                                          column.primary_key ? QStringLiteral(" [PK]") : QString());
        ...
        qInfo().noquote() << QStringLiteral("Inserted country id=%1").arg(demoCountry.id);

        demoCountry.name = "Showcase-Country-Updated";
        if (!ctx.countries.UpdateById(demoCountry, demoCountry.id))
            return fail("showcase crud", ctx.countries.GetLastError());
        ctx.countries.Select().Where(QString("id = %1").arg(demoCountry.id)).ShowList();
        ...
        showcaseValue("distinct countries", ctx.cities.Select().Distinct().Count("country_id"));
        ...
        {
            qInfo().noquote() << "[JOIN] RIGHT/FULL OUTER JOIN skipped: SQLite does not support them";
        }
        ...
        qInfo().noquote() << QString::fromUtf8(ctx.cities.Select().Limit(2).ToJson());

        showcaseSection("CHANGE TRACKING");
        const QList<City> trackedRows = ctx.cities.Select().Where("name = 'New York'").ToList();
        if (!trackedRows.isEmpty())
        {
            City tracked = trackedRows.first();
            qInfo().noquote() << QStringLiteral("tracked=%1, dirty=%2")
                                     .arg(ctx.cities.IsTracked(tracked) ? "true" : "false")
                                     .arg(ctx.cities.IsDirty(tracked) ? "true" : "false");
            tracked.name = "New York-Tracked";
            qInfo().noquote() << "changed columns:" << ctx.cities.GetChangedColumns(tracked).join(", ");
        ...
        showcaseValue("ExecuteScalar count",
                      ctx.cities.ExecuteScalar(QString("SELECT COUNT(*) FROM %1").arg(quotedCities)));
        const QList<QJsonObject> rawRows = ctx.cities.ExecuteQuery(
            QString("SELECT %1, %2 FROM %3 ORDER BY %1")
                .arg(quotedId, quotedName, quotedCities));
        qInfo().noquote() << QStringLiteral("ExecuteQuery rows: %1").arg(rawRows.size());
        if (!rawRows.isEmpty())
            qInfo().noquote() << QString::fromUtf8(QJsonDocument(rawRows.first()).toJson(QJsonDocument::Compact));
        ...
        if (!conn.RollbackTransaction())
            return fail("showcase transaction", conn.ErrorMessage());
        qInfo().noquote() << "BeginTransaction -> CommitTransaction -> RollbackTransaction: PASS";
    ...
    bool seed()
    {
        ...
        if (!ctx.cities.Insert(ny) || !ctx.cities.Insert(la) || !ctx.cities.Insert(toronto))
    ...
    bool testSql()
    {
        ...
        return check(pg.GetDatabasesSQL().contains("pg_database"), "sql postgres")
            && check(ms.AddTableSQL(cities).contains("IDENTITY"), "sql sqlserver")
            && check(sqlite.AddTableSQL(cities).contains("AUTOINCREMENT"), "sql sqlite");
    }
    ...
    bool testCrud()
    {
        ...
        const QList<City> rows = ctx.cities.Select().Where(QString("id = %1").arg(seattle.id)).ToList();
        ...
        return check(ctx.cities.Select().Where(QString("id = %1").arg(seattle.id)).Count() == 0, "crud verify");
    }
    ...
    bool testBulkCrud()
    {
        ...
        if (!check(ctx.countries.Select().Where("name LIKE 'Bulk-%-updated'").Count() == 2,
                   "bulk update verify"))
            return false;
        ...
        return check(ctx.countries.Select().Where("name LIKE 'Bulk-%'").Count() == 0,
                     "bulk delete verify");
    }
    ...
    bool testAggregate()
    {
        return check(ctx.cities.Select().Count() == 3, "agg count")
            ...
            && check(ctx.cities.Select().Distinct().Count("country_id") == 2, "agg distinct");
    }

    bool testQuery()
    {
        if (!check(ctx.cities.Select().Where("name = 'New York'").Count() == 1, "query where"))
            return false;
        if (!check(ctx.cities.Select().Where("name = 'New York'")
                                  .OrWhere("name = 'Toronto'")
                                  .Count() == 2, "query or where"))
            return false;

        const QList<City> byName = ctx.cities.Select().OrderByAsc("name").Limit(2).ToList();
        if (!check(byName.size() == 2, "query limit"))
            return false;

        const QList<City> page = ctx.cities.Select().OrderByAsc("id").Skip(1).Take(1).ToList();
        if (!check(page.size() == 1, "query pagination"))
            return false;

        if (config.driver != Q1Driver::SQLITE)
        {
            if (!check(!ctx.countries.Select().RightJoin("cities", "countries.id = cities.country_id").ToList().isEmpty(),
                       "query right join"))
                return false;
        }

        if (config.driver == Q1Driver::POSTGRE_SQL || config.driver == Q1Driver::SQLSERVER)
        {
            if (!check(!ctx.cities.Select().FullJoin("countries", "cities.country_id = countries.id").ToList().isEmpty(),
                       "query full join"))
                return false;
        }

        return check(!ctx.cities.Select().LeftJoin("countries", "cities.country_id = countries.id").ToList().isEmpty(),
                     "query left join");
    }
    ...
    bool testJsonAndInclude()
    {
        ...
        const QList<City> cities = ctx.cities.Select().Include("countries").ToList();
        if (!check(cities.size() == 3, "include rows"))
            return false;

        const QJsonArray result = ctx.cities.GetLastJson();
        return check(!result.isEmpty() && result.first().toObject().contains("countries"),
                     "include countries");
    }
    ...
    bool testThreads()
    {
        ...
        for (int i = 0; i < 4; ++i)
        {
            ...
            {
                Q1Connection local(settings.driver, settings.host, settings.database, settings.user, settings.password, settings.port);
                if (!local.Connect())
                {
                ...
                {
                    std::lock_guard<std::mutex> lock(mutex);
                    failures.push_back(QStringLiteral("worker %1 count %2").arg(i).arg(count));
                }
                local.Disconnect();
            });
        }
        ...
        return check(failures.empty(), failures.empty() ? "threads" : failures.front());
    }
...
int main(int argc, char* argv[])
{
    ...
    for (const DbConfig& config : dbs)
    {
        ...
        line(QStringLiteral("%1").arg(config.tag), ok ? "PASS" : "FAIL", ok ? "32" : "31");
        if (!ok)
            out << "  reason: " << suite.lastError << Qt::endl;
        failures += ok ? 0 : 1;
    }

    line("All databases", failures == 0 ? "PASS" : "FAIL", failures == 0 ? "32" : "31");
